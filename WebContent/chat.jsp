...
<body class="container-fluid py-1">
        ...
        <div class="row">
                ...
                <a href="#" title="Menu">
                    <i class="mx-3 text-dark fas fa-ellipsis-v text-secondary"></i>
									...
									<c:forEach var="user" items="${listUser}">
										...
										<a style="text-decoration: none;" href="onechat?id=<c:out value='${user.username}' />&type=one">
										<div class="mx-2 p-1">
          ...
          <p style="font-size: 12px" class="mx-2 my-0 p-0">last message of user</p>
                            		...
                            		<c:forEach var="group" items="${listGroup}">
										...
										<a style="text-decoration: none;" href="groupchat?id=<c:out value='${group.groupId}' />&name=<c:out value='${group.groupName}' />&type=group">
										<div class="mx-2">
			<div class="d-flex justify-content-start align-items-center p-1">
			<img width="10%" class="rounded-circle" alt="100x100" src="https://mdbootstrap.com/img/Photos/Avatars/img%20(30).jpg"
          data-holder-rendered="true">
          ...
          <p style="font-size: 12px" class="mx-2 my-0 p-0">last message of group</p>
			...
			<%if(type == null){ %>
						...
						<div class="card-group border-0 m-0">
							<div class="shadow border-0 card p-0" style="height: 600px">
									...
									<div class="d-flex flex-column justify-content-between align-items-center">
										<h3>Welcome to Whatsapp Web</h3>
										
										<img width="50%" alt="" src="https://raw.githubusercontent.com/anandhamurthy/Whatsapp-Web/main/chat.gif">
									</div>
								</div>
							</div>
						</div>
		  	...
		  	<div class="col-md-8 m-0 p-0">
						<div class="card-group border-0 m-0">
							<div class="shadow border-0 card p-0" style="height: 600px">
								<div style="background: url(https://raw.githubusercontent.com/anandhamurthy/Whatsapp-Web/main/bg.png)" class="card-body p-0" >
          ...
          <p class="mx-2 my-0 p-0"><b><%= name %></b></p>
          <div class="d-flex flex-row mx-2">
									<c:forEach var="users" items="${listMembers}">
									<p style="font-size: 12px" class="mx-1 my-0 p-0">${users},</p>
										
									</c:forEach>
									</div>
		  <%} else {%>
			    <p class="mx-2 my-0 p-0"><b><%= id %></b></p>
			    <p style="font-size: 12px" class="mx-2 my-0 p-0">online</p>
			<% } %>
					...
					<div class="
					  ...
					  <div><b><c:out value="${mes.from}" /></b></div>
					  <img class="myImg" src="UploadImgFile?filename=<c:out value='${mes.message}' />" alt="" width="250px">
			...
			<br />
				...
				<c:if test="${mes.type == 'DOC' }">	
					...
					<div class="
						...
						my-2
					  " style="border-radius: 10px">
					  <div><b><c:out value="${mes.from}" /></b><i style="font-size:12px;" class="mx-2 far fa-file"></i></div>
				...
				<c:if test="${mes.type == 'AUDIO' }">	
					...
					<div class="
					  ...
					  <audio controls>
					        ...
					        <source src="UploadAudFile?filename=<c:out value='${mes.message}' />">
    				  </audio>
		...
		<div style="background: #EBEDEF" class="d-flex  justify-content-start align-items-center p-2">
											...
											id="message" 
												type="text"
												name="message"
												class="border-0 form-control" placeholder="write message.." />
									...
									<button id="recordButton" class=" border-0 rounded-0 btn btn-default px-3" ><i class="fas fa-microphone"></i></button>
												...
												<button class=" border-0 rounded-0 btn px-3" onclick="sendMessage('<%= id %>', '<%= current_username %>')" type="button"><i class="fas fa-paper-plane"></i></button>
											</span>
										</div>
	...
	function openNewGroup() {
		window.location.replace("new_group");
	}
	...
	function close() {
		...
		var websocket = new WebSocket("ws://localhost:8085/Whatsapp-Web/chat");
		console.log(websocket);
		websocket.binaryType = "arraybuffer";
		
    	
    	websocket.onopen = function() {
    		var message = { messageType: 'LOGIN', message: '<%= id %>', to: '<%= id %>', from : '<%= current_username %>', type: '<%= type %>'};
	...
	function sendMessage(username, current_username) {
		...
		if (websocket != null && websocket.readyState == 1) {
			...
			var encrypted_text = CryptoJS.AES.encrypt(message.value, '');
			
			var messageItem = { messageType: 'MESSAGE', message: message.value, frontmessage: message.value, to: username, from: current_username, type: '<%= type %>' };
			...
			if (type == 'IMAGE') {
				messageContainer.innerHTML += `<div class="d-flex justify-content-end">
					<div style="background-color: #DAF7A6; border-radius: 15px" class="
						...
						my-2
					  " >
						<img class="myImg" src="UploadImgFile?filename=`+msg+`" alt="" width="250px">
				...
				messageContainer.innerHTML += `<div class="d-flex justify-content-end">
			        ...
			        <source src="UploadAudFile?filename=`+msg+`">
			  </audio>
				...
				if (type == 'IMAGE') {
					...
					messageContainer.innerHTML += `<div class="d-flex justify-content-start">
						<div class="
							...
							my-2
						  " style="border-radius: 10px">
						  <div><b>`+from+`</B><i style="font-size:12px;" class="far fa-file"></i></div>
						  <a href="UploadDocFile?filename=`+msg+`">
	...
	function uploadDocFile(file) {
	    ...
	    .then(response => {
	    	if (response.status == 200) {
	    		var message = { messageType: 'DOC', message: file.name, frontmessage: file.name, to: '<%= id %>', from : '<%= current_username %>', type: '<%= type %>'};
	...
	async function uploadImgFile(file) {
	    ...
	    .then(response => {
	    	if (response.status == 200) {
	    		var message = { messageType: 'IMAGE', message: file.name, frontmessage: file.name,  to: '<%= id %>', from : '<%= current_username %>', type: '<%= type %>'};
	...
	async function getByteArray(myFile) {
	    ...
	    console.log(byteArray);
			...
			if (websocket != null && websocket.readyState == 1) {
	      		...
	      		var messageItem = { messageType: 'IMAGE', message: myFile.name, file: byteArray, to: '<%= id %>', from : '<%= current_username %>', type: '<%= type %>' };
	      		console.log(messageItem);
	...
	async function uploadAudFile(blob) {
	    ...
	    .then(response => {
	    	if (response.status == 200) {
	    		...
	    		var message = { messageType: 'AUDIO', message: filename, frontmessage: file.name, to: '<%= id %>', from : '<%= current_username %>', type: '<%= type %>'};
	...
	function getMessage(msg, from, to) {
		
		var bytes  = CryptoJS.AES.decrypt(msg.toString(), 'whatsappweb');
